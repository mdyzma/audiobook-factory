...
| `just audition <voice> <book=""> <role=""> <text="">` | Reads a passage from a book in that voice, at the settings the book will actually be narrated with, and records everything that produced the sample. Without a book it says a fixed sentence. Samples are named by what made them, so asking twice costs nothing. |
...
| `just pronounce <slug> <written=""> <spoken="">` | How this book says a word. With a slug alone, shows the dictionary and every passage it changes. With a word and its spoken form, adds it. With a word alone, removes it. Takes effect at the next `just chunk`. |
...
| `just synth <slug> <voice=""> <device="auto">` | Renders every fragment. Resumable: re-run to continue after an interruption. `voice` defaults to the cast in `config/cast.yml`; name one to narrate the whole book in it. |
